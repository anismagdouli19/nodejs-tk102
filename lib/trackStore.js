...
const TrackStore = class TrackStore {
    ...
    addData(data) {
        ...
        let foundIndex = this.tracks.findIndex(track => track.imei === data.imei);

        if (foundIndex >= 0) {
            this.tracks[foundIndex].gpsData.push(data.gpsData);
            clearTimeout(this.tracks[foundIndex].timeout);
            this.tracks[foundIndex].timeout = setTimeout(this.finishTrack, 1500, {
                this: this,
            ...
            this.tracks[index].timeout = setTimeout(this.finishTrack, 15000, {
                this: this,
                trackIndex: index,
                emitter: this.trackEmitter
            });
