...
const specsLKGPS = [
    function (data) {
        ...
        // *HQ,8120016933,V5,084930,A,4706.0382,N,01525.4618,E,003.50,333,5,60:38:E0:F8:AA:8E,-74,BC:85:56:0D:0E:77,-75,64:7C:34:59:4F:1F,-85,06:7C:34:59:4F:1F,-86,D8:84:66:C7:17:E8,-91,232,1,0,5,7615,39616,26,7615,39626,22,7605,
        let text = data.text;
        let result = null;
        let str = [];
        let datetime = '';
        ...
        try {
            ...
            if (str.length >= 18 && str[0] === '*HQ' && (str[2] === 'V1' || str[2] === 'V5') && str[4] === 'A') {
                datetime = Date.now();
                ...
                result = {
                    ...
                    imei: str [1],
                    valid: str[4] === 'A' ? true : false,
                    speed: str[10]*0.51444444444
                };
            }
...
/**
 * Parses the raw hex buffer...
 * because PAJ Tracker mainly sends data in hex encoded binary format
 *
 * Example of incoming buffer and parsing:
...
const specsLKGPSbinary = [
    function (data) {
        ...
        try {
            // last byte of longitude:
            // contains: bit7654 = last place of longitude; bit0:  not defined
            //                                              bit1:  1:A, 0:V
            //                                              bit2:  1: north lat, 0: south lat
            //                                              bit3:  1: east long, 0: west long
            let combinedField = (raw.slice(0x15, 0x16)[0]) & 0x0f;

            let latDir = (combinedField & 0x04) ? 'N' : 'S';
            let longDir = (combinedField & 0x08) ? 'E' : 'W';
            let validGps = (combinedField & 0x02) ? true : false;

            if (!validGps)
                return null;

            let lastDigitLongitude = raw.slice(0x15, 0x16);
            lastDigitLongitude = lastDigitLongitude[0] >>> 4;

            // parse the latitude as a string, for example: 4706.0414
            let latitudeString = raw.slice(0x0c, 0x0e).toString('hex') + '.' + raw.slice(0x0e, 0x10).toString('hex');

            // parse the longitude as a string, for example: 01525.4367
            let longitudeString = raw.slice(0x11, 0x15).toString('hex');
            longitudeString = longitudeString.substring(0,5) + '.' + longitudeString.substring(5,8) + lastDigitLongitude.toString();

            let speed = raw.slice(0x16, 0x19).toString('hex').substring(0,2);
            let direction = raw.slice(0x16, 0x19).toString('hex').substring(3,5);
            ...
            result = {
                ...
                geo: {
                    ...
                    latitude: tk102.dmm2ddd(latitudeString, latDir),
                    longitude: tk102.dmm2ddd(longitudeString, longDir)
                },
                valid: validGps,
                speed: speed*0.51444444444,
                direction: direction
            };
