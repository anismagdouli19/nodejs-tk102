        ...
        try {
            ...
            if (str.length === 13) {
                ...
                result = {
                    ...
                    datetime: datetime,
                    geo: {
                        longitude: tk102.fixGeo(gpsData [0], gpsData [1]),
                        latitude: tk102.fixGeo(gpsData [2], gpsData [3])
                    },
                    imei: str [1]
                };
            }
...
tk102.parse = function (raw) {
    ...
    if (raw.charAt(0) === '#') {
        ...
        parsedData = specsDrsPaj;
    } else {
        parsedData = specs;
    }

    while (data === null && i < parsedData.length) {
        data = parsedData [i](raw);
        i++;
    }
 ...
 */
const tk102_string = '1203292316,0031698765432,GPRMC,211657.000,A,5213.0247,N,00516.7757,E,0.00,273.30,290312,,,A*62,F,imei:123456789012345,123';
const newOne = '*HQ,8120016933,V1,141825,A,4706.0098,N,01525.4893,E,001.00,247,280218,FFFFFBFF,232,01,0,0,3#';
